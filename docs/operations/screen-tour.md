...
| Hardware profiles | What does a team request? Compare the CPU, single-GPU, four-GPU, and MIG-oriented profiles. | A profile describes a request. Only the observed physical hardware and completed runtime tests prove availability. L40S cannot provide MIG. |
...
| Feature store → Datasets | Which historical training snapshots are registered? Inspect a saved dataset when one is present. | A feature definition and an online lookup do not automatically create a saved training dataset. An empty dataset list must be explained, not called populated. |
