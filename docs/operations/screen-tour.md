...
| Observe & monitor → Dashboard → Cluster | Is the platform healthy? Inspect System health and resource trends. | System health reports Ready nodes. Cluster-wide summaries and project breakdowns have different scope; the [native dashboard guide](native-dashboards.md) explains their denominators. |
| Dashboard → Models | Which deployment served the request? Select the project and Model deployment. | Model deployments, Replica count, P90 latency, and token throughput provide the serving overview. A blank CPU quota cell is not zero usage. |
| Dashboard → LLM Traffic | How much work reached the model? Match the serving project and model to the request window. | Throughput (req/s) and Token throughput (tokens/s); Error rate is a configured server-side series, not every client or access-control failure. |
| Dashboard → LLM Performance | When does generation begin, and is cache reused? | Inspect TTFT, E2E, KV cache hit rate, and KV cache usage. Skip the inter-token panel: its installed query uses a missing metric and falls back to zero. Local cache reuse does not prove distributed KV transfer. |
| Dashboard → LLM Utilization | Are replicas busy or waiting? | GPU utilization, Requests running, and Requests waiting; use replica legends and the recorded interval. GPU collection does not cover every earlier benchmark. |
| Dashboard → Usage | Who consumed the model under which limit? Select one subscription. | Token consumption chart/table and rate-limited decisions. Success rate describes the limiter, not complete inference success; summary and chart use different time windows. |
...
| MLflow → aurora-assistant-demo → Overview → Usage | What did the application request consume? Match the trace interval. | The measured six-trace snapshot has 9,950 provider-reported tokens. No cost data means no recorded price evidence, not free inference. |
| MLflow → aurora-assistant-demo → Overview → Tool calls | Which tools actually executed? Inspect usage and latency charts. | Two successful calls were indexed from one request after the exporter correction. Older artifact traces were not backfilled. Follow the [MLflow guide](../labs/mlflow.md) for a new test drive. |
...
| AI hub → MCP servers | What can an agent do? Open the Aurora Supply server and inspect its tools. | Catalog, inventory, and forecast-backed replenishment tools support the recommendation. Policy retrieval uses Knowledge/RAG. No purchase-execution tool is provided. |
