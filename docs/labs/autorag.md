...
The later inventory-playbook revision 1.1 clarifies the existing tool’s reorder-point floor and separates target stock from additional quantity. Its current content prefix is `corpus/90f7574ec40ae483`. That update has not been re-scored by AutoRAG: the table above belongs to the immutable `corpus/d7897df084530cf0` inputs and original generation configuration. Both input revisions and the measured artifacts are preserved.
