...
## Run notebook 10 with a supervising presenter

**Live rehearsal passed on September 22, 2026.** The actual notebook completed all three code cells in 67.773 seconds, with **32/32 benchmark requests**, zero errors or incomplete requests, and no remaining benchmark worker, GuideLLM process, or port-forward after cleanup. The measured comparison ran at **13:55:06–13:55:55 UTC**. See the [dated result and interpretation](ahead-llmd.md#live-notebook-result-september-22-2026-1355-utc); this successful execution does not establish a routing speedup.

Open `10-llmd-live.ipynb` with **Aurora Inference Demo**. Its **Run All** action requests a new comparison and displays only that request ID's result. First, the presenter must start the temporary administrator-side controller using the [AHEAD-LLMD instructions](ahead-llmd.md#supervised-live-comparison-from-the-workbench). If its health is stale, expired, or unavailable, stop and resolve the session; the notebook never substitutes historical measurements.

The controller runs **GuideLLM 0.6.0 on the administrator workstation**. It uses temporary loopback forwards to the two existing Qwen backends; the Workbench receives no administrator credential, port-forward, or new backend permission. This is separate from notebooks 07–09 and their persistent inference environment. Notebook 10 uses that kernel only for its request and result interface; it does not install or run the local controller's GuideLLM there.

Each comparison has two fixed phases: explicit **round-robin**, then **llm-d**, each capped at **30 seconds, 16 requests, concurrency 2, and 32 output tokens**. One additional authorized request capped at 16 output tokens, plus one denied anonymous request, checks authentication. A fully completed comparison can therefore include **33 successful requests**, of which 32 belong to the benchmark phases.

Only one comparison runs at a time, with a **30-second cooldown**, a **six-run session budget**, and an administrator-selected expiry no more than three hours away. The worker has a **180-second wall budget** including its shutdown margin; the notebook waits at most **210 seconds**. **Interrupt kernel** writes a cancellation request. Five-second polling plus a bounded channel call can take up to about nine seconds to detect it; cleanup then stops the controller's benchmark children and forwards. Closing the browser alone is not cancellation, and a server may finish a request it already accepted.

Inspect actual counts and errors before comparing TTFT, backend assignments, EPP requests, and prefix-cache counters. The phases use distinct fresh salts with the same prompt structure, so prompt bytes differ. The administrator baseline bypasses the normal Gateway, while llm-d includes authentication and scheduling. Shared traffic, existing cache contents, and these different paths prevent a causal speedup claim. Use the new result's timestamps in the native Qwen dashboards; dated workshop results remain separate evidence.
