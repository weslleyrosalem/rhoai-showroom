...
The native catalog search was also inspected: the 4B result shows the **Aurora Supply · Qwen curated** source and opens the upstream model card. This imported card leaves tensor type and parameter size as `N/A` and offers a license agreement link. Use the pinned runtime manifest and the estimates above to discuss the actual deployment; do not turn an absent catalog field or absent safety insight into a validated score.
