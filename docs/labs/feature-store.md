...
Select the dataset to review its event-time boundaries and feature references. Then open **Connected workbenches → Aurora Supply — Data Science Lab** and run Notebook 06. The `opendatahub.io/feast-config: aurora_supply` annotation records that actual, tested connection. The project may also show Feast's internal dummy entity; it is not a ninth Aurora product.
