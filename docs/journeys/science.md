...
Use the [native Playground](../labs/playground.md) to compare plain Llama chat with Qwen tool calling. The Qwen exercise retrieves an actual policy file, then calls governed MCP inventory tools using the visitor's authorized identity. A chat-capable model is not automatically a tool-capable model.
