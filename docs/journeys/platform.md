...
5. **14–18 minutes: operate.** Open **Observe & monitor → Dashboard**. Use **LLM Traffic**, **LLM Performance**, and **LLM Utilization** for the selected serving project/model, then **Usage** for the selected MaaS subscription. Follow the [native dashboard guide](../operations/native-dashboards.md) for exact panels and windows. Relate the request to token, latency, and GPU metrics. Distinguish the current request from a previously recorded experiment. Application traces and infrastructure metrics answer different questions.
...
| Playground | Can the assistant use the synthetic inventory fixture and policy together? | Ask for a replenishment recommendation for `AS-001`; inspect the tool's exact SKU, forecast provenance, and required approval. No purchase is executed. |
| Observe & monitor → Dashboard | Is the platform processing real work? | Use the installed Models and three LLM tabs for serving behavior, Usage for subscription accounting, and Cluster for capacity context. Follow the [native chart map](../operations/native-dashboards.md). Quota decisions, inference outcomes, and answer quality are different measurements. |
