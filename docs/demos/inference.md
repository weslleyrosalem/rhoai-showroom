...
| 25–30 min | Hardware profiles and capacity guard | Explain how one GPU, four-GPU tensor parallelism, and multiple replicas change deployment choices. Show prepared manifests and their honest readiness status; discuss what the next controlled comparison would measure. |
...
- **vLLM versus Transformers:** not measured in this rehearsal. A valid comparison uses the same GPU, model revision, precision, token lengths, concurrency, and measurement window.
